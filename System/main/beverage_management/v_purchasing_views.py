...
@login_required
def purchasing_export_dashboard_pdf_view(request):
    if request.user.role != 'purchasing_officer' and not request.user.is_superuser:
        messages.error(request, 'Access denied.')
        return redirect('home')
    
    # 1. Gather Data (Matches dashboard view)
    pending_requisitions = RequisitionForm.objects.filter(route_destination='purchasing').exclude(status='closed').count()
    active_pos = PurchaseOrder.objects.filter(status__in=['ordered', 'in_transit']).count()
    total_suppliers = Supplier.objects.filter(is_active=True).count()
    
    recent_requisitions = RequisitionForm.objects.filter(route_destination='purchasing').exclude(status='closed').order_by('-date_created')[:10]
    recent_pos = PurchaseOrder.objects.all().order_by('-date_created')[:10]
    ...
    # 2. Render PDF
    template_path = 'purchasing_officer/dashboard_pdf.html'
    template = get_template(template_path)
    html = template.render(context)
    
    response = HttpResponse(content_type='application/pdf')
    filename = f"Purchasing_Report_{timezone.now().strftime('%Y-%m-%d')}.pdf"
...
@login_required
def purchasing_export_orders_list_pdf_view(request):
    if request.user.role != 'purchasing_officer' and not request.user.is_superuser:
        messages.error(request, 'Access denied.')
        return redirect('home')

    # Get all orders (matching the logic in orders.html)
    orders = PurchaseOrder.objects.all().order_by('-date_created')
    ...
    # Render PDF
    template_path = 'purchasing_officer/orders_list_pdf.html'
    template = get_template(template_path)
    html = template.render(context)
    
    response = HttpResponse(content_type='application/pdf')
    filename = f"PO_Master_List_{timezone.now().strftime('%Y-%m-%d')}.pdf"
    response['Content-Disposition'] = f'attachment; filename="{filename}"'
