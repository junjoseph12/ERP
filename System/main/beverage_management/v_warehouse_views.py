...
@login_required
def warehouse_export_dashboard_pdf_view(request):
    if request.user.role != 'warehouse_manager' and not request.user.is_superuser:
        messages.error(request, 'Access denied.')
        return redirect('home')
    
    # 1. Gather Data (Identical logic to dashboard_view)
    total_items = InventoryItem.objects.count()
    low_stock_count = InventoryItem.objects.filter(quantity__lte=F('reorder_point')).count()
    out_of_stock_count = InventoryItem.objects.filter(quantity__lte=0).count()
    ...
    # Get Lists
    recent_activity = StockAdjustment.objects.all().order_by('-timestamp')[:15] # Showing slightly more for PDF
    low_stock_items = InventoryItem.objects.filter(quantity__lte=F('reorder_point'))
    ...
    # 2. Render PDF
    template_path = 'warehouse_manager/dashboard_pdf.html'
    template = get_template(template_path)
    html = template.render(context)
    
    response = HttpResponse(content_type='application/pdf')
    filename = f"Warehouse_Report_{timezone.now().strftime('%Y-%m-%d')}.pdf"
...
@login_required
def warehouse_export_inventory_pdf_view(request):
    if request.user.role != 'warehouse_manager' and not request.user.is_superuser:
        messages.error(request, 'Access denied.')
        return redirect('home')

    # 1. Apply Filters (Same as main inventory view)
    ...
    template_path = 'warehouse_manager/inventory_list_pdf.html'
    template = get_template(template_path)
    html = template.render(context)
    
    response = HttpResponse(content_type='application/pdf')
    filename = f"Inventory_List_{timezone.now().strftime('%Y-%m-%d')}.pdf"
    response['Content-Disposition'] = f'attachment; filename="{filename}"'
...
@login_required
def warehouse_export_receiving_pdf_view(request):
    if request.user.role != 'warehouse_manager' and not request.user.is_superuser:
        messages.error(request, 'Access denied.')
        return redirect('home')
    ...
    # 1. Gather Data
    incoming_pos = PurchaseOrder.objects.filter(status='in_transit').order_by('date_created')
    received_history = ReceivingReport.objects.all().order_by('-date_received')
    ...
    template_path = 'warehouse_manager/receiving_list_pdf.html'
    template = get_template(template_path)
    html = template.render(context)
    
    response = HttpResponse(content_type='application/pdf')
    filename = f"Receiving_Report_{timezone.now().strftime('%Y-%m-%d')}.pdf"
    response['Content-Disposition'] = f'attachment; filename="{filename}"'
