...
def staff_requisition_detail_view(request, req_id):
    ...
    # Attach a temporary 'total_price' attribute to each item for the template
    for item in req_items:
        # Use selling price, fallback to cost price, fallback to 0
        price = item.item.selling_price if item.item.selling_price else (item.item.cost_price or 0)
        item.temp_total = price * item.quantity_requested
        grand_total += item.temp_total

    # Check for download flag
    auto_download_receipt = request.session.pop('download_delivery_receipt_id', None)

    context = {
        'req': req,
        'req_items': req_items, # Pass the processed items
...
def staff_confirm_delivery_view(request, req_id):
    ...
    # Only process if not already closed to prevent duplicate logic
    if req.status != 'closed':
        ...
        messages.success(request, f"Requisition {req.rf_number} marked as DELIVERED.")
    
    # SET SESSION FLAG FOR PDF DOWNLOAD
    request.session['download_delivery_receipt_id'] = req.id
    
    return redirect('staff_requisition_detail', req_id=req.id)

@login_required
def staff_download_receipt_pdf_view(request, req_id):
    req = get_object_or_404(RequisitionForm, id=req_id)
    ...
    for item in req.items.all():
        price = item.item.selling_price if item.item.selling_price else (item.item.cost_price or 0)
        total_line = price * item.quantity_requested
        grand_total += total_line
        
        # Attach custom attributes for the template
        item.total_line_price = total_line
    ...
    context = {
        ...
        'grand_total': grand_total,
        'generated_at': timezone.now(),
        'delivery_ref': f"DR-{req.id:04d}"
    }
    
    template_path = 'staff/delivery_receipt_pdf.html'
    template = get_template(template_path)
    html = template.render(context)
    
    response = HttpResponse(content_type='application/pdf')
    response['Content-Disposition'] = f'attachment; filename="Receipt_{req.rf_number}.pdf"'
